...
for (let i = 0; i < button.length; i++) {
    button[i].innerText = "OOPS, I AM UPSIDE DOWN";
...
for (let i = 0; i < divs.length; i++) {
    setInterval(() => {
        divs[i].style.backgroundColor = colors[Math.floor(Math.random() * 5)];
    }, 1000);
}

//changing fonts in divs
let fonts = ["arial", "Times New Roman", "Lucida Console", "Courier New"];
for (let i = 0; i < divs.length; i++) {
    setInterval(() => {
        divs[i].style.fontFamily = fonts[Math.floor(Math.random() * 4)];
    }, 1000);
}